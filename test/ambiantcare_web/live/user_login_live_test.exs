defmodule AmbiantcareWeb.UserLoginLiveTest do
  ...
  describe "Log in page" do
    test "renders log in page", %{conn: conn} do
      {:ok, _lv, html} = live(conn, ~p"/en/users/log_in")
  ...
  describe "user login" do
    test "redirects if user login with valid credentials", %{conn: conn} do
      ...
      {:ok, lv, _html} = live(conn, ~p"/en/users/log_in")
    ...
    test "redirects to login page with a flash error if there are no valid credentials", %{
      ...
      assert redirected_to(conn) == "/en/users/log_in"
    end
  end
  ...
  describe "login navigation" do
    ...
    test "redirects to forgot password page when the Forgot Password button is clicked", %{
      ...
      {:ok, conn} =
        ...
        |> render_click()
        |> follow_redirect(conn, ~p"/en/users/reset_password")
