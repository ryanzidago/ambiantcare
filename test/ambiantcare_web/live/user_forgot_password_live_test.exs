defmodule AmbiantcareWeb.UserForgotPasswordLiveTest do
  ...
  describe "Forgot password page" do
    test "renders email page", %{conn: conn} do
      {:ok, lv, html} = live(conn, ~p"/en/users/reset_password")

      assert html =~ "Forgot your password?"
      assert has_element?(lv, ~s|a[href="#{~p"/en/users/register"}"]|, "Register")
      assert has_element?(lv, ~s|a[href="#{~p"/en/users/log_in"}"]|, "Log in")
    end
  ...
  describe "Reset link" do
    ...
    test "sends a new reset password token", %{conn: conn, user: user} do
      {:ok, lv, _html} = live(conn, ~p"/en/users/reset_password")
