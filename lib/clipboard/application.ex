defmodule Clipboard.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      ClipboardWeb.Telemetry,
      Clipboard.Repo,
      {DNSCluster, query: Application.get_env(:clipboard, :dns_cluster_query) || :ignore},
      {Phoenix.PubSub, name: Clipboard.PubSub},
      # Start the Finch HTTP client for sending emails
      {Finch, name: Clipboard.Finch},
      # Start a worker by calling: Clipboard.Worker.start_link(arg)
      # {Clipboard.Worker, arg},
      # Start to serve requests, typically the last entry
      ClipboardWeb.Endpoint
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: Clipboard.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    ClipboardWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
